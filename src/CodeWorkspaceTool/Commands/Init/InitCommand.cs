...
public sealed class InitCommand(IWorkspaceRepository repository) : Command<InitCommandSettings>
{
    protected override int Execute(CommandContext context, InitCommandSettings settings, CancellationToken cancellationToken)
    {
        ...
        var fileName = $"{name}{WorkspaceFile.Extension}";
        ...
        repository.Save(new WorkspaceDocument(), path);
