...
CI (`.github/workflows/ci.yml`) runs the same steps plus test coverage, published to the workflow run summary.

The solution has three projects:

- `src/CodeWorkspaceTool` — the tool itself.
- `test/CodeWorkspaceTool.Tests` — unit tests (NUnit, FakeItEasy, AutoFixture) with fakes for the file-system boundary.
- `test/CodeWorkspaceTool.Integration.Tests` — tests that run the real `CommandApp` against real temp directories.
